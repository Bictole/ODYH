...
public class Inventory : MonoBehaviour
{
    ...
    */
    ...
    {
        foreach (BagItem bag in bags)//Checks all bags
        {
            if (bag.BagScr.AddBagItem(item)) //Tries to add the item
            {
    ...
    private bool PlaceInStack(Item item)
    {
        foreach (BagItem bag in bags)//Checks all bags
        {
            foreach (Slot slots in bag.BagScr.slotscrList) //Checks all the slots on the current bag
            {
    ...
    {
        bags[bagIndex].BagScr.slotscrList[slotIndex].AddItem(item);
    }
    ...
    public int ItemCount(string type)
    {
        ...
        foreach (BagItem bag in bags)
        {
            foreach (Slot slot in bag.BagScr.slotscrList)
            {
                if (!slot.Empty && slot.TheItem.Title == type)
                {
    ...
    public Stack<Item> GetItems(string type, int count)
    {
        ...
        foreach (BagItem bag in bags)
        {
            foreach (Slot slot in bag.BagScr.slotscrList)
            {
                if (!slot.Empty && slot.TheItem.Title == type)
                {
                    foreach (Item item in slot.itemStack)
                    {
                        items.Push(item);
